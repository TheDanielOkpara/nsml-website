  ...
  <style>
    ...
    .article-hero-overlay {
      ...
      background: linear-gradient(
        to bottom,
        rgba(13,31,60,0.2)  0%,
        rgba(13,31,60,0.15) 30%,
        rgba(13,31,60,0.55) 65%,
    ...
    /* ── ARTICLE BODY ───────────────────────── */
    /* Reading column: centred, constrained to ~680px */
    ...
    /* WP: the_content() renders into .article-body */
    .article-body {
      grid-column: 2;
    }
    ...
    /* Inline images — slightly wider than the text column */
    ...
    .article-body .article-inline-img img {
      width: 100%;
      display: block;
      height: 400px;
      object-fit: cover;
    ...
    .article-body .article-inline-img figcaption {
      font-size: 0.8125rem;
      color: var(--text-muted);
      padding: 0.75rem 0 0;
      line-height: 1.5;
    ...
    /* ── AUTHOR CARD ────────────────────────── */
    /* WP: rendered using get_the_author_meta() */
    .author-card {
      grid-column: 2;
      margin-top: 3rem;
      padding: 0.25rem;
    ...
    @media (max-width: 768px) {
      /* Hero — switch to static flow on mobile so long titles
         can't extend upward into the fixed nav                */
      .article-hero {
        height: auto;
        min-height: 0;
        max-height: none;
        padding-top: 5.5rem;   /* clears the floating nav pill */
        display: flex;
        flex-direction: column;
    ...
    @media (max-width: 480px) {
      .article-body .stats-callout { grid-template-columns: 1fr; }
    }
  </style>
</head>
<body>
  ...
  <!-- ── ARTICLE HERO ──────────────────────────
       WP: Use get_the_post_thumbnail_url() for bg image
           Use the_title() for heading
  ...
  <!-- ── STICKY META BAR ───────────────────────
       WP: the_date(), get_the_author(), estimated reading time via plugin
  ─────────────────────────────────────────── -->
  ...
  <!-- ── ARTICLE CONTENT ──────────────────────
       WP: the_content() renders all of this
           WordPress block editor (Gutenberg) produces
           identical HTML structure automatically.
  ─────────────────────────────────────────── -->
  <div class="article-content-wrap">
    ...
    <!-- ── AUTHOR CARD ─────────────────────────
         WP: get_the_author_meta('display_name')
             get_the_author_meta('description')
             get_avatar()
    ──────────────────────────────────────── -->
    <div class="author-card">
      <div class="author-card-inner">
        <!-- WP: get_avatar(get_the_author_meta('ID'), 64) -->
        <div class="author-avatar">NS</div>
        <div>
          <!-- WP: get_the_author_meta('display_name') -->
          <div class="author-name">NSML Editorial Team</div>
          <!-- WP: custom author role field via ACF -->
          <div class="author-role">Nilayo Sports Management Ltd</div>
          <!-- WP: get_the_author_meta('description') -->
          <p class="author-bio">The NSML Editorial Team covers sporting events, partnership announcements, and industry insights from across Africa's sports management landscape. For press enquiries, contact the NSML Communications team.</p>
        </div>
      </div>
    </div>
  ...
  <div style="background:var(--navy);padding:5rem 1.5rem;border-top:3px solid var(--green);">
    <div style="max-width:42rem;margin:0 auto;text-align:center;" data-reveal>
      <div class="eyebrow-pill" style="justify-content:center;display:inline-flex;margin-bottom:1.5rem;background:rgba(26,184,60,0.12);border-color:rgba(26,184,60,0.3);color:var(--green);">
        <span class="eyebrow-dot"></span>
        Stay Updated
      </div>
      <h2 style="font-family:var(--font-d);font-size:clamp(1.75rem,3.5vw,2.5rem);font-weight:800;letter-spacing:-0.03em;color:#ffffff;margin-bottom:1rem;line-height:1.1;">
        Never Miss an <span style="color:var(--green)">NSML Story</span>
      </h2>
      <p style="font-size:1.0625rem;color:rgba(255,255,255,0.55);margin-bottom:2.25rem;line-height:1.7;">
        Event dates, results, partnership announcements, and insights — delivered straight to your inbox.
      </p>
      <form id="articleNlForm" novalidate style="display:flex;flex-direction:column;gap:0.625rem;max-width:32rem;margin:0 auto;">
        <div style="display:flex;flex-wrap:wrap;gap:0.75rem;">
          <input type="email" id="articleNlEmail" placeholder="Your email address"
            style="flex:1;background:rgba(255,255,255,0.08);border:1.5px solid rgba(255,255,255,0.15);border-radius:9999px;padding:0.875rem 1.5rem;font-family:var(--font-b);font-size:0.9375rem;color:#ffffff;outline:none;transition:border-color 0.3s;"
            onfocus="this.style.borderColor='var(--green)'"
            onblur="this.style.borderColor=this.value&&!/invalid/.test(this.dataset.state||'') ? 'var(--green)' : 'rgba(255,255,255,0.15)'"
            autocomplete="email"
            aria-label="Email address">
          <button type="submit" id="articleNlBtn" class="btn btn-fill" style="white-space:nowrap;flex-shrink:0;min-width:0;">
            <span id="articleNlLabel">Subscribe</span>
            <span class="btn-icon">↗</span>
          </button>
        </div>
        <p id="articleNlError" style="font-size:0.8125rem;color:#ff8a80;font-weight:500;display:none;text-align:center;"></p>
        <div id="articleNlSuccess" style="display:none;align-items:center;gap:0.625rem;justify-content:center;background:rgba(26,184,60,0.15);border:1px solid rgba(26,184,60,0.35);border-radius:9999px;padding:0.75rem 1.5rem;">
          <span style="color:var(--green);font-size:1rem;">✓</span>
          <span style="font-size:0.9375rem;color:#ffffff;font-weight:600;">You're subscribed — thanks!</span>
        </div>
      </form>
    </div>
  </div>
  ...
  <script>
    ...
    const io = new IntersectionObserver((entries) => {
      entries.forEach((entry, i) => {
        ...
        setTimeout(() => {
          el.style.transition = `opacity 0.75s cubic-bezier(0.32,0.72,0,1) ${i * 80}ms,
                                  transform 0.75s cubic-bezier(0.32,0.72,0,1) ${i * 80}ms`;
          el.style.opacity = '1';
    ...
    (function initShare() {
      ...
      if (btnX) btnX.href = `https://twitter.com/intent/tweet?url=${pageUrl}&text=${pageTitle}&via=nilayosports`;

      // LinkedIn
      const btnLI = document.getElementById('shareLI');
      if (btnLI) btnLI.href = `https://www.linkedin.com/shareArticle?mini=true&url=${pageUrl}&title=${pageTitle}&source=Nilayo+Sports+Management`;

      // WhatsApp
      const btnWA = document.getElementById('shareWA');
      if (btnWA) btnWA.href = `https://api.whatsapp.com/send?text=${pageTitle}%20${pageUrl}`;
    })();
      ...
      copyBtn.addEventListener('click', () => {
        navigator.clipboard.writeText(window.location.href)
          ...
          .catch(() => {
            ...
            setTimeout(() => { copyBtn.innerHTML = '&#x2398;'; }, 2200);
          });
      });
    }

    /* ── ARTICLE NEWSLETTER VALIDATION ─────── */
    const anlForm    = document.getElementById('articleNlForm');
    const anlEmail   = document.getElementById('articleNlEmail');
    const anlError   = document.getElementById('articleNlError');
    const anlSuccess = document.getElementById('articleNlSuccess');

    if (anlForm) {
      anlForm.addEventListener('submit', (e) => {
        e.preventDefault();
        const val = anlEmail.value.trim();
        const isValid = /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(val);

        if (!val) {
          anlError.textContent   = 'Please enter your email address.';
          anlError.style.display = 'block';
          anlEmail.style.borderColor = '#ff8a80';
          return;
        }
        if (!isValid) {
          anlError.textContent   = 'Please enter a valid email address.';
          anlError.style.display = 'block';
          anlEmail.style.borderColor = '#ff8a80';
          return;
        }
      ...
      anlEmail.addEventListener('input', () => {
        if (anlError.style.display !== 'none') {
          anlError.style.display = 'none';
          anlEmail.style.borderColor = 'rgba(255,255,255,0.15)';
        }
      });
    }
    ...
    /* ── STICKY META BAR: hide behind nav ─────
       Adjusts top offset so meta bar sits below the floating nav
    ──────────────────────────────────────── */
    const metaBar = document.querySelector('.article-meta-bar');
    const nav     = document.getElementById('mainNav');
    if (metaBar && nav) {
      const setOffset = () => {
        metaBar.style.top = nav.offsetHeight + 'px';
      };
      setOffset();
